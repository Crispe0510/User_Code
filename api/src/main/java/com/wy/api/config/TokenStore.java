...
public class TokenStore {
    ...
    private static final Map<String, Long> ADMIN_TOKENS = new ConcurrentHashMap<>();
    // h5Token -> h5UserId
    ...
    public static String createAdminToken(Long userId) {
        ...
        ADMIN_TOKENS.put(token, userId);
        return token;
    }

    public static Long getAdminUserId(String token) {
        return token == null ? null : ADMIN_TOKENS.get(token);
    }

    public static void removeAdminToken(String token) {
        if (token != null) ADMIN_TOKENS.remove(token);
    }
